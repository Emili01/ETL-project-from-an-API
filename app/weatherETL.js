 ...
 async function saveToMongo(database, municipio, data) {
   try {
 ...
 async function dataExistsForMunicipio(database, municipio) {
   const collection = database.collection('datos_meteorologicos');
 ...
 export async function fetchAndSaveAllData(database) {  
  
   try {
     for (const municipio of municipios) {
       const datosExisten = await dataExistsForMunicipio(database, municipio);
       if (datosExisten) {
       ...
       if (data) {
         await saveToMongo(database, municipio, data);
       }
       await delay(1000);
     }
   } catch (error) {
